...
def buildsRepo = "https://github.com/thelastpickle/cassandra-builds.git"
def buildsBranch = "mck/17869"
    ...
    testTargets.each {
        ...
        if (exists(branchName, targetName)) {
            matrixJob("${jobNamePrefix}-${targetName}") {
                ...
                steps {
                    shell("""
                            ./cassandra-builds/build-scripts/cassandra-test-docker.sh thelastpickle mck/17869_v2/trunk ${buildsRepo} ${buildsBranch} ${testDockerImage} ${targetName} \${split}${_testSplits} ;
        ...
        dtestTargets.each {
            ...
            if (exists(branchName, targetName)) {
                matrixJob("${jobNamePrefix}-${targetArchName}") {
                    ...
                    steps {
                        shell("""
                            ./cassandra-builds/build-scripts/cassandra-dtest-pytest-docker.sh thelastpickle mck/17869_v2/trunk https://github.com/thelastpickle/cassandra-dtest.git mck/17869 ${buildsRepo} ${buildsBranch} ${dtestDockerImage} ${targetName} \${split}/${splits} ;
                            wget --retry-connrefused --waitretry=1 "\${BUILD_URL}/timestamps/?time=HH:mm:ss&timeZone=UTC&appendLog" -qO - > console.log || echo wget failed ;
                            xz -f console.log
                            """)
                    }
    ...
    if (exists(branchName, 'cqlsh-tests')) {
        matrixJob("${jobNamePrefix}-cqlsh-tests") {
            ...
            publishers {
                ...
                steps {
                    shell("""
                        ./cassandra-builds/build-scripts/cassandra-test-docker.sh thelastpickle mck/17869_v2/trunk ${buildsRepo} ${buildsBranch} ${testDockerImage} cqlsh-test ;
                        wget --retry-connrefused --waitretry=1 "\${BUILD_URL}/timestamps/?time=HH:mm:ss&timeZone=UTC&appendLog" -qO - > console.log || echo wget failed ;
                        xz -f console.log
                        """)
                }
